...
3. Alpha Vantage MCP - дает Claude доступ к акциям, ETF, equity options,
   fundamentals, indicators, news, macro, FX, crypto и commodities.
...
Эта инструкция уже проверялась на macOS. На Windows порядок должен быть таким
же, но Windows-проверка пока должна считаться отдельным smoke-test.
...
В текущем Claude Desktop скиллы и MCP-подключения открываются через кнопку `+`
под полем ввода, но это два разных пути:

- Для MCP и connectors: `+` -> `Connectors` -> `Manage Connectors` -> `+`.
...
6. Выберите установку MCP, extension или server.
7. Выберите файл `databento-mcp-desktop-extension.mcpb`.
8. Claude попросит Databento API key.
9. Откройте файл `databento_api_key.txt`.
10. Скопируйте ключ из файла.
11. Вставьте ключ в защищенное поле Claude.
12. Нажмите Save.
13. После сохранения включите тумблер Enable для Databento MCP.
...
- Claude понимает, что:
  - CME futures и CME futures options идут через Databento.
  - Акции, ETF, equity options, fundamentals, indicators, news, macro, FX,
    crypto и commodities идут через Alpha Vantage.
...
1. Установить `.mcpb` файл Databento MCP и вставить Databento key.
